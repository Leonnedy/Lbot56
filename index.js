...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:LeoBlade\n' // Su nombre
            + 'ORG:Lbot55;\n' // Nombre del bot
            + 'TEL;type=CELL;type=VOICE;waid=18292097057:+1 829 209 7057\n' // Numero de Whatsapp
            + 'END:VCARD'
prefix = '*','#','+','.',',','???','-'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `�hola mi querido saltamontes @${num.split('@')[0]}\n�Se bienvenido a  *${groupName}*\n\nLee las reglas o habran consecuencias ;)`
				...
				teks = `buey buey buey ... buey de cara negra si te fuiste y te sentarás y sobre tu cabeza�@${num.split('@')[0]} 𝚔𝚔𝚔𝚔𝚔𝚔𝚔 😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					wait: 'Perate, recuerda que mi internet es lento',
					success: 'Pronto',
					...
					levelnol: '*ERROR* °-°',
					error: {
				stick: 'No pude convertir la foto/video en sticker porque la vida es triste.',
				Iv: 'Link invalido'
				},
				only: {
					group: 'Este comando solo se puede usar en grupos q.q',
					premium: `Ei {pushname2} Solamente usuarios PREMIUM pueden usar esto*`,
					mod: 'ESTA ORDEN ES ESPECÍFICA PARA EL MODERADOR DE LA ⚡SUPER ANO⚡*',
					benned: 'Has sido baneado, contacta con el propietario para deshacerte de ti.',
					ownerG: 'Solo Leo/Laura puede usar este comando mijo',
					ownerB: 'Solo Leo/Laura puede usar esse comando mijo',
					userB: `──「 LISTA 」──\nOlá ${pushname} !\nNo estás registrado como amigo de mi propietario, así que pídele que te agregue como amigo.\n\n──「 ⚡Super Ano⚡ 」──`,
					admin: 'Este comando solo puede ser usado por admins!',
					Badmin: 'Este comando solo se puede usar cuando el ⚡Dueño⚡ se convierte en ADM del grupo crack',
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["18292097057","18292097057"] // Recoloque o seu numero
			const mod = [ownerNumber,"5491128667535","5491128667535"]// Moderador do bot
			const adminbotnumber = ["14254419777","14254419777"]// admin bot numero
			const frendsowner = ["528441249591","528441249591"]// amigo do criador 
			const premium = ["528441249591","528441249591","18292097057","5491128667535","14254419777","18292097057"]
...
if (text.includes("placa"))
  ...
  let hasil = ` *🔍CONSULTA REALIZADA🔍* \n\n ➸ *AÑO:*  ${res.data.ano}\n ➸ *AÑO MODELO* : ${res.data.anoModelo}\n ➸ *CHASIS* : ${res.data.chassi}\n ➸ *CÓDIGO DE RETORNO* : ${res.data.codigoRetorno}\n ➸ *CÓDIGO DE SITUACIÓN* : ${res.data.codigoSituacao}\n ➸ *COR* : ${res.data.cor}\n ➸ *MARCA* : ${res.data.marca}\n ➸ *MUNICIPIO* : ${res.data.municipio}\n ➸ *SITUACION* : ${res.data.situacao}\n ➸ *UF* : ${res.data.uf}\n *📌BY:May Bot*` 
  conn.sendMessage(id, hasil, MessageType.text); 
 })
 }		        

        if (messagesC.includes("prietototote")){
		if (!isGroup) return
		if (!isAntiRacismo) return
		if (isGroupAdmins) return reply('hombre, no digas estas cosas, está mal, pero si te portas bien no seras baneado')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`chau racista ${sender.split("@")[0]} serás expulsado de este grupo en 5 segundos`)
		setTimeout( () => {
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("5 segundos, a tu casa gorda puta 🤙")
		}, 0)
	}
		...
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`tchau racista ${sender.split("@")[0]} Seras expulsado de este grupo en 5 segundos`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 5000)
		setTimeout( () => {
		...
		setTimeout( () => {
	        ...
	        if (messagesC.includes("prietoinmundo")){
		if (!isGroup) return
		if (!isAntiRacismo) return
		if (isGroupAdmins) return reply('hombre, no digas estas cosas, está mal, pero si te portas bien no seras baneado')
		client.updatePresence(from, Presence.composing)
		...
		setTimeout( () => {
	        ...
	        if (messagesC.includes("porque eres negrosma qwq?")){
		if (!isGroup) return
		if (!isAntiRacismo) return
		if (isGroupAdmins) return reply('hombre, no digas estas cosas, está mal, pero si te portas bien no seras baneado')
		client.updatePresence(from, Presence.composing)
		...
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`link detectado ${sender.split("@")[0]} Seras expulsado de este grupo en 5 segundos`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 5000)
		setTimeout( () => {
		...
		if (!isGroup) return
		if (!isAntiLink) return
		if (isGroupAdmins) return reply('eres administrador, así que no te prohibiré el uso de enlaces, 🙂')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`link detectado ${sender.split("@")[0]} Seras expulsado de este grupo en 5 segundos`)
		...
		setTimeout( () => {
			...
			reply("5 segundos, a tu casa gorda puta 🤙")
	         ...
	         if (messagesC.includes("yo he peleado contra mi tio")){
			client.updatePresence(from, Presence.composing)
			reply("y me ha ganado por su pito, que pesa 500 kilos")
	}
	
                 if (messagesC.includes("me ha dado la vuelta al culo y lo vio todo el mundo")){
			client.updatePresence(from, Presence.composing)
			reply("Y ahora me mide 10 metros")

	}
	         if (messagesC.includes("pero de profundo")){
			client.updatePresence(from, Presence.composing)
			reply("Mi abuela no respira, esta dura como una roca")

	}
	         if (messagesC.includes("Le digo que se despierte pero se hace la loca")){
			client.updatePresence(from, Presence.composing)
			reply("Mi creatividad vuela como su cenizas")

	}
	        if (messagesC.includes("Puedo hacerte una parodia hasta una cesión con visa")){
			client.updatePresence(from, Presence.composing)
			reply("*aplausos*")
		...
		if (messagesC.includes("Buen dia")){
			client.updatePresence(from, Presence.composing)
			reply("☀️")
	}
		...
		if (messagesC.includes("hentai")){
			client.updatePresence(from, Presence.composing)
			reply("akbron, este grupo es cristiano")
	}
						
		if (messagesC.includes("merda")){
			client.updatePresence(from, Presence.composing)
			reply("y quien dijo que este es mi problema?")
	}
	
		if (messagesC.includes("osi")){
			client.updatePresence(from, Presence.composing)
			reply("ono")
	}

		if (messagesC.includes("Leo")){
			client.updatePresence(from, Presence.composing)
			reply("Oe, mi dueño probablemente esté ocupado, así que espera uwu")
	}
	
		if (messagesC.includes("*miren")){
			client.updatePresence(from, Presence.composing)
			reply("aveer")
	}
			...
			if (messagesC.includes("@18292097057")){
			client.updatePresence(from, Presence.composing)
			...
			client.updatePresence(from, Presence.composing) 
	     	const d = fs.readFileSync('./sticker/tatakae.webp');
            client.sendMessage(from, d, sticker, {quoted: mek})
    }  
            if (messagesC.includes("AMOGUS")){
			client.updatePresence(from, Presence.composing) 
	     	const d = fs.readFileSync('./sticker/AMOGUS.webp');
            client.sendMessage(from, d, sticker, {quoted: mek})
    }
          ...
          if (messagesC.includes("oh me vengo")){
			client.updatePresence(from, Presence.composing) 
	     	const d = fs.readFileSync('./sticker/gostoso.webp');
            client.sendMessage(from, d, sticker, {quoted: mek})
    }
          if (messagesC.includes("follar")){
			client.updatePresence(from, Presence.composing) 
	     	const d = fs.readFileSync('./sticker/eita.webp');
            client.sendMessage(from, d, sticker, {quoted: mek})
    }
          
	        if (messagesC.includes("musica de sexo")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/Sexo.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
				if (messagesC.includes("stay with me")){
			client.updatePresence(from, Presence.composing)
			...
			if (messagesC.includes("AMOGUS")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/AMOGUS.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
			...
			if (messagesC.includes("mmm worship")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/worship.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
		
			if (messagesC.includes("musica de atraco")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/tmnc.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
				if (messagesC.includes("chitpos")){
			client.updatePresence(from, Presence.composing)
		...
		if (messagesC.includes("Te amo")){
			client.updatePresence(from, Presence.composing)
			reply("me empalagan, y eso que soy un inshe bot uwu")
	}
	
		if (messagesC.includes("¿Alguien para rolear?")){
			client.updatePresence(from, Presence.composing)
			reply("quisiera pero soy solo un bot😢")
	}
			...
			switch(command) {
	            ...
	            case 'help':
                    ...
                    putagg = await getBuffer(`https://ibb.co/1zzg5Pp`)
		...
		if (!isGroup) return reply("este comando solo puede ser usado en grupos")
			reply(`Aqui los comandos de ⚡Lbot55⚡ ${pushname}`)
			setTimeout( () => {
			...
			setTimeout( () => {
				client.updatePresence(from, Presence.composing)
				reply("git clone https://github.com/Leonnedy/Lbot55")
			}, 7000)
			setTimeout( () => {
				client.updatePresence(from, Presence.composing)
				reply("cd Lbot55")
			...
			setTimeout( () => {
				...
				reply("Diviértete crack😎")
				    ...
				    try {    
						...
						reply('EITA VEI FUE ERROR AQUÍ MN')
					}
					break
					case 'lista':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply('¿Quién eres tú, el dueño?')
					if (args.length < 1) return reply('¿Dónde está el texto compa?')
					reply(mess.wait)
					client.sendMessage(from, 'mensaje agregado exitosamente a la lista' , text, { quoted: mek })
                    ...
                    case 'lindos':
                     if (!isGroup) return reply(mess.only.group)
                      ...
                      reply(`*${pushname2} Estas son las mas bonitas del grupo\n${groupName}\n\n@554791347623\n@${cuzao11.jid.split('@')[0]} \n@${cuzao12.jid.split('@')[0]} \n@${cuzin21.jid.split('@')[0]} \n@${cuzin22.jid.split('@')[0]} \n\n Cortesia de Super Xandão`)
                      membr.push(cu11.jid)
                      membr.push(cu12.jid)
				...
				case 'ramaljadian':
                   ...
                   case 'gcpf':
                    ...
                    break
				case 'ytmp4':
					if (args.length < 1) return reply('¿Dónde está la url wey?')
		...
		case 'iri':
			...
			break
                ...
                case 'gneon':
					...
					gbl1 = gh.split("/")[0];
					gbl2 = gh.split("/")[1];
					if (args.length < 1) return reply('¿Soy un jodido vidente? donde esta el texto')
           ...
           case 'playmp3':
            ...
            case 'smule':
                ...
                case 'chitpos':
                 ...
                 randKey = jsonData[randIndex];
                hasil = await getBuffer(randKey.result)
                sendImage(hasil, mek, '*todas son re putas*')
				break
				 case 'naruto':
...
// recursos anime, random @✞︎𝙼𝚎𝚕𝚒𝚘𝚍𝚊𝚜✞︎ domina vida
			...
			break
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `*MUSICA ENCONTRADA!!!*\nTítulo : ${anu.result.title}\nUrl : ${anu.result.source}\nTamaño : ${anu.result.size}\n\n*Espera mientras descargo, no spamees en el chat :)*`
                buffer = await getBuffer(anu.result.thumbnail)
                lagu = await getBuffer(anu.result.url_audio)
                client.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3})
                client.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${anu.title}.mp3`, quoted: mek})
          ...
          reply('Espere..')
				...
				case 's':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envíe fotos con subtítulos **sticker* o etiquete una imagen que ya ha sido enviada`)
					}
					break
				case 'tts':
					if (args.length < 1) return client.sendMessage(from, '¿dónde está el código del idioma mn? \n Exemplo: *tts es puto el que lo lea', text, {quoted: mek})
					...
					rano = getRandom('.ogg')
					dtt.length > 100
					? reply('La mayor parte del texto es una mierda')
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							fs.unlinkSync(ranm)
							buff = fs.readFileSync(rano)
							if (err) return reply('falla:(')
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('No eres Miembro Premium, contacta con Leo/May/Aiden o escribe *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
					...
					case 'addvip':  
					if (!isOwner) return reply(mess.only.ownerB)
					if (!isPremium) return reply('¡No eres un miembro Premium, contacta ✞︎Leo✞︎ o ingresa * # Daftarvip * para comprar acceso Premium!' ,text, { quoted: mek })
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('La marca objetivo que quieres lanzar!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = '╭────「 *PREMIUM👑* 」──*\n│+ *Número* : \n│+ *Expirado*: *30 Days*\n│+ *Status*: *ATIVO*\n│ gracias por actualizar para premium🥰\n*╰──────「 *posição* 」────'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
				...
				case 'neko':
					...
					break	
					case 'cekvip': 
					if (!isPremium) return reply('No eres Miembro Premium, contacta con el propietario o ingresa *.Daftarvip* para adquirir acesso Premium!' ,text, { quoted: mek })
					me = client.user
					uptime = process.uptime()
					client.sendMessage(from,  `*──────────────────*\n*Nombre de bot:* ✞︎Lbot55✞︎ BOT\n*─────────────────*\n『 *𝐕𝐈𝐏 𝐔𝐒𝐄𝐑*』\n*──────────────────*\n*•Número:* *${sender.split("@s.whatsapp.net")[0]}*\n*•Status:* *ATIVO*\n*──────────────────*\n*Status Bot:* *${kyun(uptime)}*\n\n*ERES MIEMBRO PREMIUM* 🐊🚩\n*──────────────────*` , text, { quoted: mek, })
					break
					case 'dellvip':
					if (!isOwner) return reply(mess.only.ownerB)
					if (!isPremium) return reply('No eres Miembro Premium, contacta con el propietario o ingresa * # Daftarvip * para adquirir o acesso Premium!' ,text, { quoted: mek })
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = '╭────「 *PREMIUM👑* 」──*\n│+ *Número* : \n│+ *Status*: *DEATIVO*\n│ Nos vemos para el próximo pedido🙂\n*╰──────「 *posicion* 」────'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`╭────「 *PREMIUM👑* 」──*\n│+ *Número* : @${mentioned[0].split('@')[0]}\n│+ *Status*: *DEATIVO*\n│ Nos vemos para el próximo pedido🙂\n*╰──────「 *posicion* 」────`, mentioned, true)
					client.sendMessage(from, mentioned)
				    }
					break
					case 'ichiadmin':
					tod = await getBuffer(`https://i.ibb.co/XDwBVDJ/1f2652c622fa.jpg`)
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT ✞︎Lbot55✞︎ ✨* 」\n*│+ wa.me/18292097057*╰──────*「 *✞︎Lbot55✞︎* 」*────*\n\n*_SI QUIERES SER ADMINISTRADOR DEL BOT✞︎Lbot55�✞︎_*\n*_Tipo / publicidad_*' })
					break
				case 'iklan':
				...
				if (!isUser) return reply(mess.only.userB)
					sa = await getBuffer(`https://ibb.co/vJZScW3`)
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/18292097057/*╰──────*「 * ✞︎Lbot✞︎* 」*────*\n\n*_SI QUIERES SER UN USUARIO PREMIUM DEL BOT ✞︎Lbot55�✞︎ BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
 
					if (!isOwner) return reply(mess.only.ownerB)
                 if (!ismod) return reply('Aún no estás registrado como usuario de Modbot')
                reply('Te has registrado como usuario de Modbot')
                break
                    case 'modbotlist':
					teks = 'Esta es la lista de usuarios facheros 🤑 :\n'
					for (let p of mod) {
						teks += `~> @${p.split('@')[0]}\n`
					}
					...
					if (!isOwner) return reply(mess.only.ownerB)
					premium = args[0]
					reply(`Comando aceptado agregar usuario premium : ${premium}`)
					break
                 case 'calculadora':
				...
				case 'porno':
					...
					memein = await kagApi.memeindo()
                                        buffer = await getBuffer(`https://cdn5-images.motherlessmedia.com/images/D25C6F4.jpg`)
					buffer = await getBuffer(`https://external-preview.redd.it/P-_u19Si4BvV5Y_EvsTR4jrdihNQFceW-ARnWPCGaWE.jpg?width=640&crop=smart&auto=webp&s=a238a883fd318e3f3ccd35140d703f7b33ecc985`)
					buffer = await getBuffer(`https://external-preview.redd.it/QM5PUZQHSCt0VGk3-9xG7DiFk6zGZBjhYt7RW8vvd6I.jpg?auto=webp&s=43515754799fdc275f25cbcce3f8e3d345293c18`)
					buffer = await getBuffer(`https://41.media.tumblr.com/tumblr_mcm0dufEVZ1rjlcdso1_500.jpg`)
					buffer = await getBuffer(`https://40.media.tumblr.com/59c01852613f9f0c3d9c267f50671b67/tumblr_njpztlEtY41u5ocruo1_1280.jpg`)
					buffer = await getBuffer(`https://i.pinimg.com/736x/a3/9d/c5/a39dc513fe196bbc2886a83fa7856620--anime-sexy-hot-anime.jpg`)
					buffer = await getBuffer(`http://hostave4.net/hpw/fhg/photo/0136picsh_tyb/01b.jpg`)
                                        buffer = await getBuffer(`https://cdn.pichunter.com/323/7/3237750/3237750_5_o.jpg`)
					buffer = await getBuffer(`https://lh3.googleusercontent.com/proxy/A-Mqr-riaFGwb6HIUKWl-7Ufx_V6mOTVkEoF8MiMuH9vxGgQuaU-BABF-D7QEw6QYjVk9rY3ifIH1hggLKoB2z1CZjuwx65zF_gyzKaKzHf1VEhXQJ1u`)
					buffer = await getBuffer(`http://i.redd.it/k464ahwrkz451.jpg`)
					buffer = await getBuffer(`https://cdn77-pic.xnxx-cdn.com/videos/thumbs169xnxxll/fb/e7/2e/fbe72edf488862ea72c35d4550ff48c6/fbe72edf488862ea72c35d4550ff48c6.26.jpg`)
					buffer = await getBuffer(`https://static.hentai-gif-anime.com/upload/20160529/16/32064/1.gif`)
					buffer = await getBuffer(`https://static.hentai-gif-anime.com/upload/20160504/9/17498/detail.gif`)
					buffer = await getBuffer(`https://static.hentai-gif-anime.com/upload/20200814/79/161742/detail.gif`
					buffer = await getBuffer(`https://static.hentai-gif-anime.com/upload/20170418/21/41555/thumbnail.gif`)
					buffer = await getBuffer(`https://static.hentai-gif-anime.com/upload/20170418/21/41554/thumbnail.gif`)
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSqWoYHFA_SDgUip6MtvimWRdub_lshAn5Edg&usqp=CAU`)
					buffer = await getBuffer(`https://static.hentai-gif-anime.com/upload/20170416/21/41481/thumbnail.gif`)
					buffer = await getBuffer(`https://static.hentai-gif-anime.com/upload/20170419/21/41570/thumbnail.gif`)
					buffer = await getBuffer(`https://static.hentai-gif-anime.com/upload/20170419/21/41591/detail.gif`)
					buffer = await getBuffer(`https://1.bp.blogspot.com/-RNk2C9lbaig/WRaFm7EeadI/AAAAAAAAAsI/_DffpfZfE9QEsrQoi1208TIwuPCcvPTRACLcB/s640/tumblr_n6ltu3UcxN1rvphqeo1_500.gif`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'heheheheheh'})
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'heheheheheh'})
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'heheheheheh'})
					buffer = await getBuffer(`https://podofilia.net/wp-content/uploads/2019/05/hentai-gifs-11.gif`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'heheheheheh'})
					buffer = await getBuffer(`https://lh3.googleusercontent.com/proxy/-b2yEDZUC0wcdYKoiKq3clH-NCOMtWuZ1QeM7Z4qU_UqWlSWwvi8CoZdCsNjEsDQktfAqjbsUJk5n9BOmMji_1-wRr5GxmLgZaP32yjyOiaPAd7JxrblAZxz9ftSxYlt`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'heheheheheh'})
					buffer = await getBuffer(`https://i1.yourhentaipics.com/images/2/782-hentai-zml84731.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'heheheheheh'})
					buffer = await getBuffer(`https://www.gifspx.com/gifs-animados-x/gspx_pack-de-gifs-hentai-sentaditas-con-una-buena-pillada.gif`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'heheheheheh'})
					buffer = await getBuffer(`https://i0.wp.com/uncensored-hentai.top/wp-content/uploads/2020/07/anime-hentai-gif-1.gif?resize=700%2C391&ssl=1`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'heheheheheh'})
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'heheheheheh'})
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'heheheheheh'})
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'heheheheheh'})
				...
				case 'loli1':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/POnmYiY.gif`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'En caso de una investigación por parte de cualquier entidad federal o similar, no tengo ninguna participación con éste grupo o con las personas que forman parte de él, no sé cómo estoy aquí, probablemente agregado por un tercero, no apoyo ninguna acción por los miembros de éste grupo'})
					break
				case 'hentai':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/8U9GwX4.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'AH, re virgo y encima pajero'})
					break
				case 'buendia':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/iAdNYAz.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Buenos días, ustedes son increíbles ❤️'})
					break
				case 'buenatarde':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/CdU6ajX.png`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Buenas tardes maquinas 😎👍'})
					break
				case 'buenanoche':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/0U7FWLj.png`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'El chad se tiene que ir a dormir ❤️'})
					break
                case 'belle':                 
				...
				case 'belle1':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Aver👀️'})
					break
				case 'lofi':
					memein = await kagApi.memeindo()
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do ✞︎Lbot55�✞︎:*\n\n https://youtube.com/channel/UCpB3qh2Sp3K23s9a2Q-Gf-g'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRJhzKetbU3pzhoZdaIo6qBklCzwvmCCOznbg&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'jeje, te atrape con las manos en la paja enfermito'})
					break
				case 'reislin':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://ibb.co/QjHnmj8`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'a cabron🤭'})
					break
				case 'buenanoche':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/toaccK9.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'buenas nocheee banda ❤️'})
					break
				case 'buendia':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/iAdNYAz.jpg`)
				...
				case 'termux':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Terminal es un programa muy conocido en el mundo de las distribuciones de Linux. Es una herramienta que facilita enormemente las tareas relacionadas con el sistema. Ahora, ¿has pensado en usar Linux Terminal en tu Android? Esta es la propuesta de Termux. \N\n * TERMUX: UTILIZA EL TERMINAL EN TU ANDROID * \n\ nEl uso del Terminal aumenta enormemente la productividad del usuario que ya tiene un cierto nivel de conocimientos técnicos. \NCon el terminal , es posible hacer varias cosas diferentes, desde navegar entre directorios e instalar programas, hasta descomprimir archivos y monitorear procesos.'})
					break
		    	case 'grupoinfo':
                    client.updatePresence(from, Presence.composing)
                    if (!isGroup) return reply(mess.only.group)
				...
				case 'dono2':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CREADOR:* Causs\n*YOUTUBE:* https://www.youtube.com/channel/UCUrr82A7Q7ap4Ir4G-2oBUQ\n*WPP:* wa.me/+18292097057\n*INSTA:* @Leonnedy0\n\n\nEspero que te haya gustado el bot 🐊🚩\n seguire agregando cosas con el tiempo nwn ️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
                ...
                case 'lolih':
                    ...
                    break
				case 'nsfwloli':
					if (!isNsfw) return reply('*ESTE COMANDO ENVIA UNA CAPTURA DE TU CHAT A LA POLICIA AMIGO.*')
					...
					break
				case 'pronomeneu':
					if (args.length < 1) return reply('Onde está la texto, hum?')
					anu = await fetchJson(`https://mhankbarbars.herokuapp.com/api/hilih?teks=${body.slice(7)}`, {method: 'get'})
					reply(anu.result)
					break
				case 'yt2mp3':
					if (args.length < 1) return reply('Onde está la url, hum?')
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
					anu = await fetchJson(`https://mhankbarbars.herokuapp.com/api/yta?url=${args[0]}&apiKey=${apiKey}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					...
					break
				case 'tiktok':
					if (args.length < 1) return reply('𝚕𝚒𝚗𝚔 w𝚎𝚢?')
			...
			reply(mess.wait)
                ...
                case 'marcar2':
                 ...
                 case 'marcar3':
					...
					break
				case 'limpar':
					if (!isOwner) return reply('¿Quién eres?, no eres mi dueño 😂')
					anu = await client.chats.all()
					client.setMaxListeners(25)
					for (let _ of anu) {
						client.clearChat(_.jid)
					}
					reply('Se eliminaron correctamente todos los chats :)')
					break
				case 'bc':
					if (!isOwner) return reply('Quien eres? tu no eres mi dueño 😂')
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ TRANSMISIÓN DE ADVERTENCIA ]\n\n${body.slice(4)}`})
						}
						reply('Transmisión enviada con éxito')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ CHICOS MIREN ESTE AVISO ]\n\n${body.slice(4)}`)
						}
						reply('Transmisión enviada con éxito')
					}
					break
        case 'promover':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esta bien jefe. este chico aquí: @${mentioned[0].split('@')[0]} ¡ahora es administrador de grupo!`, mentioned, true)
				...
				case 'rebaixar':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esta bien jefe. este chico aquí: @${mentioned[0].split('@')[0]} perdio el adm con éxito!`, mentioned, true)
				...
				case 'banir':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('bienvenidos a su dosis diaria de baneo')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Salebai:\n'
						for (let _ of mentioned) {
				...
				case 'admins':
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('{ ❗ } *Selecciona un sticker, subnormal*')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.png')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ No se pudieron convertir los stickers en imágenes. ❌')
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: '>//<'})
						fs.unlinkSync(ran)
					})
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('Modo simi desactivado con éxito en este grupo 😡️')
					} else {
						reply('1 para activar, 0 para desactivar, lerdao vc em KKKKK')
					}
					break
				case 'bienvenido':
					if (!isGroup) return reply(mess.only.group)
					...
					if (Number(args[0]) === 1) {
						...
						reply('Ha activado con éxito la función de bienvenida en este grupo 😉️')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Ha desactivado con éxito la función de bienvenida en este grupo 😡️')
					} else {
						reply('1 para activar, 0 para desactivar, lerdão vc em KAKKKK')
					}
                                      break
				case 'clonar':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Marque a la persona que desea clonar\n\n*EXEMPLO:* clone @')
		        ...
		        case 'setfoto2':
                    ...
                    break
                case 'bc2':
					if (!isadminbot) return reply('¿Quién eres tú?')
					if (args.length < 1) return reply('.......')
					anu = await client.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
				...
				case 'setpp3':
                    if (!isGroup) return reply(mess.only.group)
                    if (!isfrendsowner) return reply('Quien sos')
                    if (!isBotGroupAdmins) return reply(mess.only.Badmin)
                    media = await client.downloadAndSaveMediaMessage(mek)
                    await client.updateProfilePicture (from, media)
                    reply('Se cambió con éxito el icono de grupo.')
                    break
                case 'bc3':
					if (!isfrendsowner) return reply('¿Quién es usted?')
					if (args.length < 1) return reply('.......')
					anu = await client.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
				...
				default:
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[ERROR]','red'), 'COMANDO NO REGISTRADO', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
